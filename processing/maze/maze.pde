...
final Maze maze = new Maze(150, 100);
final PrintWriter pw = new PrintWriter(System.out);
int n = 0;
Iterator<Integer> solutionIter;

void setup() {
  size(960, 720);
  background(0);
  frameRate(6);
  maze.layout(new Random(), pw);
  Set<Integer> solutionSet = maze.solve(0, 0);
  solutionIter = solutionSet.iterator();
}

void draw() {
  int w = Math.min(width / (maze.width + 2), height / (maze.height + 2));
  if (n++ == 0) {
    maze.print(pw, true);
    stroke(255);
    for (int y = 0; y <= maze.height; y++) {
      for (int x = 0; x < maze.width; x++) {
        if (y == maze.height || !maze.ups[y * maze.width + x]) {
          line(w + x * w, w + y * w,
            w + (x + 1) * w, w + y * w);
        }
      }
    ...
    for (int y = 0; y < maze.height; y++) {
      for (int x = 0; x <= maze.width; x++) {
        if (x == maze.width || !maze.lefts[y * maze.width + x]) {
          line(w + x * w, w + y * w,
            w + x * w, w + (y + 1) * w);
        }
      }
  ...
  if (solutionIter.hasNext()) {
    int s = solutionIter.next();
    int y = s / maze.width;
    int x = s % maze.width;
    fill(255, 255, 0);
    rect(w + x * w + 2, w + y * w + 2,
      w - 4, w - 4);
  }
}
...
class Maze {
  ...
  Set<Integer> solve(int x, int y) {
    ...
    final Deque<Direction> fromStack = new ArrayDeque<Direction>();
    final Deque<Direction> directionStack = new ArrayDeque<Direction>();
